    ...
    with left:
        ...
        with sub_tabs[0]:
            if uploaded is not None:
                ...
                # Chrome blocks data: URIs used as an <iframe> navigation target
                # (a security hardening change to stop data-URI phishing pages).
                # <embed> is treated as plugin content rather than a navigation,
                # so it isn't subject to that block.
                st.markdown(
                    f'<embed src="data:application/pdf;base64,{b64}" type="application/pdf" '
                    f'width="100%" height="520" '
                    f'style="border:1px solid rgba(255,255,255,0.07); border-radius:8px;" />',
                    unsafe_allow_html=True,
                )
                st.download_button(
                    "⬇️ Open / Download PDF",
                    ...
                    help="Use this if the inline preview above doesn't render in your browser.",
                )
            else:
